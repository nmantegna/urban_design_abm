 ...
 * -------------------------------------------------------
 * Shapefiles (place all in the project's includes/ folder):
 *
...
model art_route_simulation

global {

    // ── Shapefiles ───────────────────────────────────────────────
    file boundary_file  <- file("../includes/bid_vector.shp");
    file route_file     <- file("../includes/art_route.shp");
...
species boundary_layer {
    aspect default {
        draw shape color: rgba(0, 0, 0, 0) border: rgb(220, 50, 50) width: 3;
    }
}
...
species studio_layer {
    aspect default {
        // Outer ring
        draw circle(4.0) color: rgba(255, 200, 0, 180) border: rgb(200, 140, 0) width: 2;
        // Inner dot
...
species pedestrian skills: [moving] {
    ...
    action initialise {
        ...
        // Shuffle studios and take a subset for this agent's visit plan
    ...
    reflex dwell when: !done and dwelling {
        if time >= dwell_end {
            ...
            if current_target = nil {
                // All studios visited — agent is done
                done            <- true;
    ...
    aspect default {
        // Agent body
        draw circle(1.5) color: ped_color border: #white;

        // Pulsing ring while dwelling at a studio
        if dwelling {
            draw circle(4.0)
                color: rgba(ped_color.red, ped_color.green, ped_color.blue, 80)
